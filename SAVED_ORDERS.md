...
## Mixed-role retailer assignment extension

The existing optional `assignedRepUids` field now accepts active Owner, Admin and Field Rep profiles through the client. The field name, UID-only values, maximum 10 unique entries, schema version and missing-as-empty behavior remain unchanged. No migration, new index, Firestore rule change, user-profile read permission, dependency or Firebase service is required. Only Owner/Admin may change assignments; Field Rep creates remain Unassigned and their contact edits preserve assignments.

Assignment controls are labeled **Assigned Users** / **Edit Assigned Users**. Manager views and specific-user filters show current names, roles and active status. Inactive users show labels such as Inactive Owner; a current Viewer is marked Active but not assignable. Missing profiles show Unavailable user. Existing UIDs remain stored through inactivity, role changes or missing profiles; only newly added UIDs require a live eligible profile. No prior role is inferred or copied into retailer data. Field Reps see only **You** or **Other assigned user**, plus counts, and cannot read other user profiles.

My Retailers is now available for Owner/Admin as well as Field Rep and matches the current UID. Owner/Admin retain All Retailers as their default and retain Assigned/Unassigned/specific-user filters. Field Rep defaults and shared active-retailer access are unchanged. Assignment never changes order creator identity, historical snapshots, retailer linkage, pricing or draft behavior.

Manual checks: create and edit mixed Owner/Admin/Field Rep assignments; select My Retailers as each role; confirm manager default and specific-user filters; remove one/all assignments; make an assigned Owner/Admin inactive or change an assigned user to Viewer and verify stale display, retention and no new selection; verify Field Rep privacy labels and absence of editable controls. Check narrow layouts with longer role/status labels. Existing deployed assignment rules support this client extension; no rules release is required. Firebase Spark remains sufficient within quotas, with no payment method or paid services required.

## Territory-aware retailer management

Retailers may store optional `territory` (trimmed display text, at most 500 characters) and `territoryNormalized` (lowercase with collapsed whitespace). Missing/blank territory displays as Unassigned Territory. No migration or new collection is required; legacy records remain editable without adding these fields.

Owner/Admin can set, change or clear territory in the existing retailer editor. Suggestions come from loaded retailers, active manager-visible profiles and the current profile, with normalized duplicates removed. Field Reps see read-only territory. Their new retailers use their current profile territory, or blank when none is assigned; the save transaction reads the current profile and rules enforce that match. Later profile changes affect My Territory filtering, not stored retailers.

Territory is organizational, not an access boundary. My Territory differs from My Retailers (UID assignment). All Active Retailers remains available to reps, including cross-territory retailers. Managers can combine assignment and territory filters. Territory is searchable and displayed on cards and profiles. Manager-only mismatch notes use existing user-profile access; reps gain no access to other user profiles. Cross-territory assignments and orders remain allowed.

Retailer create/update rules consolidate equivalent manager checks into one manager-or-restricted-rep branch. Rep updates cannot change assignment, territory or normalized territory field presence or values, or active status. Existing assignment validation still permits at most 10 distinct valid UIDs. Owner/Admin retain management access. Users, orders and cigar rules remain unchanged. No new index, service, paid search or payment method is required; Spark remains sufficient within existing quotas.

Manual checks: manager set/change/clear territory; rep creation with/without home territory; My Territory versus My Retailers and cross-territory access; combined manager filters; live home-territory changes; mismatch privacy; legacy records; account switching/sign-out/role loss; narrow layouts; existing order/draft/reorder flows. Production testing of territory writes requires a separately approved rules release. Nothing has been deployed by this change.
