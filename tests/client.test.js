...
for(const name of ['getGaugePosition','Gauge','addToOrder','DetailOrderControls','AuthorizationProfileEditor','orderWholesaleTotal','orderRetailTotal','orderGrossProfit','orderMarginPct','buildOrderText','emailOrder','copyOrderText'])assert.deepEqual(clean(named(ast,name)),clean(named(oldAst,name)),name+' changed');
