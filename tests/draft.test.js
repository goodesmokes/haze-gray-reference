...
test('UID-scoped draft lifecycle isolates accounts, authorization, transitions and legacy storage',()=>{
 ...
 for(const name of ['DraftUid','OrderItems','OrderRetailer','OrderEmail','OrderNotes','ShowOrderBuilder','ShowFinalReview','ShowOrderHistory','CompareOrderId','CopyConfirmed','FormOpen','ConfirmDeleteId','ShowAuthorizedUsers','UserProfile','ProfileReady','ProfileError','User','AuthReady'])env['set'+name]=value=>state[name]=value;
 ...
 env.orderDraftKey=compile(text('orderDraftKey'));env.loadOrderDraft=compile(text('loadOrderDraft'));env.clearProtectedDraft=compile(text('clearProtectedDraft'));
 compile(authEffect)();
 const login=(uid,role='owner',active=true)=>{env.auth.currentUser=uid?{uid}:null;authCallback(env.auth.currentUser);assert.deepEqual(state.OrderItems,[]);assert.equal(state.OrderRetailer,'');if(uid)profileCallback({exists:()=>true,data:()=>({role,active})});};
 const persist=()=>Function(...Object.keys(env),'draftUid','orderItems','orderRetailer','orderEmail','orderNotes','return ('+persistEffect+')()')(...Object.values(env),state.DraftUid,state.OrderItems,state.OrderRetailer,state.OrderEmail,state.OrderNotes);
 login('owner');assert.equal(state.OrderRetailer,'');state.OrderRetailer='Owner A';state.OrderEmail='owner@example.test';state.OrderNotes='Owner notes';state.OrderItems=[{lineKey:'1982__1982-robusto__box10',cigarId:'1982',cigarName:'1982',vitola:'Robusto',dims:'50 x 5',packKey:'box10',packLabel:'10ct Box',unitPrice:64.5,retailUnitValue:124,qty:2}];persist();assert.equal(JSON.parse(storage.get(env.orderDraftKey('owner'))).orderRetailer,'Owner A');
 ...
 login('owner');assert.equal(state.OrderRetailer,'Owner A');assert.equal(state.OrderItems[0].qty,2);assert.equal(state.OrderEmail,'owner@example.test');assert.equal(state.OrderNotes,'Owner notes');login('rep','field_rep');assert.deepEqual(state.OrderItems,[]);assert.equal(state.OrderRetailer,'Rep B');
